...
# Kalshi: fixed for March 2026 API changes (dollar fields, yes_bid_dollars)
...
def fetch_kalshi_brackets(series):
    # Updated for March 2026 API changes - prices now in dollars format
    ...
    for m in markets:
        ...
        # Try all price field variants - new dollars fields and old cents fields
        yes_ask = None
        no_ask = None

        # New format (post March 2026): dollars strings like "0.45"
        yes_ask_dollars = m.get("yes_ask_dollars") or m.get("yes_bid_dollars")
        no_ask_dollars = m.get("no_ask_dollars") or m.get("no_bid_dollars")
        if yes_ask_dollars:
            try:
                yes_ask = round(float(yes_ask_dollars) * 100)
            except Exception:
                pass
        if no_ask_dollars:
            try:
                no_ask = round(float(no_ask_dollars) * 100)
            except Exception:
                pass

        # Old format fallback (integer cents)
        if yes_ask is None:
