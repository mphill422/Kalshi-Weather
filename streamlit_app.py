...
# - Optional Kalshi odds paste box with edge comparison
# - Trade confidence
...
with st.expander("Paste Kalshi ladder / odds (optional but recommended)", expanded=False):
    market_text = st.text_area(
        "Paste lines like: 88 to 89 +127  OR  88 to 89 56c",
        height=140,
...
df_sources["Forecast High"] = df_sources["Forecast High"].apply(lambda x: fmt_num(x, 1))
...
if not obs_hist.empty and obs_temp is not None:
    ...
    if temp_30m_ago is not None:
        heating_rate_30m = obs_temp - temp_30m_ago
    if temp_60m_ago is not None:
        heating_rate_60m = obs_temp - temp_60m_ago
...
for r in rows:
    ...
    if r["Bracket"] in market_prices:
        ...
        item["Model - Market Edge"] = f"{edge*100:+.1f}%"
        if best_edge is None or edge > best_edge:
            best_edge = edge
            best_edge_bracket = r["Bracket"]
...
if best_edge is not None and best_edge < MIN_EDGE_TO_BET:
    reasons.append(f"Best market edge too small ({best_edge*100:+.1f}% < {MIN_EDGE_TO_BET*100:.0f}%)")
...
if reasons:
    ...
    if best_edge_bracket is not None:
        st.success(f"BET SIGNAL: {best_edge_bracket} | edge {best_edge*100:+.1f}%")
    else:
